...
BEGIN
	...
	BEGIN TRY
		...
		-- Step 3, check if image needs to be associated with Proof of Work
		BEGIN
			SET @ErrorCode = 220
			SET @ErrorMessage = 'dbo.GetProofOfWork Failed'
			SET @DeviceRequestID = (
					SELECT ID
					FROM dbo.GetProofOfWork(@DeviceID)
				)

			IF (@DeviceRequestID IS NOT NULL)
			BEGIN
				SET @ErrorCode = 221
				SET @ErrorMessage = 'usp_InsertDeviceRequestImage Failed'

				--associate image with proof of work or other event
				EXEC usp_InsertDeviceRequestImage @DeviceRequestID,
					@DeviceImageID
			END
		END

		-- Step 4, check if image needs to be associated with DeviceRequest
		BEGIN
			SET @ErrorCode = 230
			SET @ErrorMessage = 'dbo.fn_GetDeviceRequestID Failed'
			SET @DeviceRequestID = (
					SELECT DeviceRequestID
					FROM dbo.fn_GetDeviceRequestID(@DeviceID)
				)

			IF (@DeviceRequestID IS NOT NULL)
			BEGIN --associate image with proof of work or other event
				SET @ErrorCode = 231
				SET @ErrorMessage = 'usp_InsertDeviceRequestImage Failed'

				EXEC usp_InsertDeviceRequestImage @DeviceRequestID,
					@DeviceImageID --send notification photo upload completed (specific user if applicable)

				SET @ErrorCode = 232
				SET @ErrorMessage = 'usp_utl_CreateNotification Failed'

				EXEC usp_utl_CreateNotification @DeviceRequestID,
					@DeviceImageID,
					@CreatedUserID
			END
		END --set success
